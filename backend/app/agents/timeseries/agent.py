...
from app.agents.timeseries.schemas import CodeGenResult
...
class TimeSeriesAgent(BaseAgent):
    ...
    def _parse_table(output: str) -> tuple[list[str], list[list[str]]]:
        """Parse the formatted table output from DatabaseAgent into columns + rows."""
    ...
    def _to_dataframe(columns: list[str], rows: list[list[str]]) -> pd.DataFrame:
        """Convert parsed table data into a pandas DataFrame with numeric coercion."""
    ...
    def _df_summary(df: pd.DataFrame, max_sample_rows: int = 5) -> str:
        ...
        lines.append(f"Columns and dtypes:")
        for col in df.columns:
    ...
    def _build_codegen_messages(
        self, question: str, df_summary: str
    ) -> list[dict[str, str]]:
        return [
            {"role": "system", "content": resolve_prompt("ts_codegen_system")},
            {
                "role": "user",
                "content": resolve_prompt("ts_codegen_user").format(
                    question=question,
    ...
    def _build_interpret_messages(
        ...
        return [
            {"role": "system", "content": resolve_prompt("ts_interpret_system")},
            {
                "role": "user",
                "content": resolve_prompt("ts_interpret_user").format(
                    question=question,
                    code=code,
    ...
    def _generate_and_execute_code(
        ...
        """Generate Python code, execute it, retry on failure.

        Returns (code, computation_result_dict).
        """
        summary = self._df_summary(df)
        messages = self._build_codegen_messages(question, summary)
        config = GenerateConfig(temperature=0)

        retry_tpl = resolve_prompt("ts_codegen_retry")
        last_code = ""

        for attempt in range(1, MAX_CODEGEN_RETRIES + 2):  # +2 because range is exclusive
    ...
    def execute(self, input_text: str, context: dict | None = None) -> AgentResult:
        ...
        command_messages = self._build_ts_command_messages(question)
        ...
        if not columns or not rows:
            return AgentResult(
                output="Error: No data returned from database for analysis.",
        ...
        if "error" in exec_result:
            return AgentResult(
                output=f"Error during analysis: {exec_result['error']}",
                metadata={
    ...
    def execute_stream(
        ...
        # Step 1: Generate DB instruction
        yield {"type": "thinking", "content": "Menyusun instruksi pengambilan data...\n"}
        command_messages = self._build_ts_command_messages(question)
        command_response = self.llm.generate(
            messages=command_messages, config=GenerateConfig(temperature=0)
        )
        ...
        columns, rows = self._parse_table(db_result.output)
        if not columns or not rows:
            yield {"type": "content", "content": "Error: No data returned from database for analysis."}
            return
        ...
        yield {"type": "thinking", "content": "Menghasilkan kode analisis...\n"}
        summary = self._df_summary(df)
        messages = self._build_codegen_messages(question, summary)
        config = GenerateConfig(temperature=0)
        retry_tpl = resolve_prompt("ts_codegen_retry")
        ...
        for attempt in range(1, MAX_CODEGEN_RETRIES + 2):
            if attempt > 1:
                yield {
                    "type": "thinking",
                    "content": f"Retry kode analisis (percobaan {attempt})...\n",
                }
            ...
            yield {"type": "thinking", "content": f"Menjalankan kode analisis...\n"}
        ...
        if exec_result is None or "error" in exec_result:
            ...
            yield {"type": "content", "content": f"Error during analysis: {error_msg}"}
            return

        yield {
            "type": "thinking",
            "content": "Analisis selesai. Menyusun interpretasi...\n",
        }
        ...
        # Emit _result for planner to capture
        result = AgentResult(
            output="",  # content was already streamed
            metadata={
                "db_instruction": db_instruction,
                "code": last_code,
                "computation_result": exec_result.get("result"),
